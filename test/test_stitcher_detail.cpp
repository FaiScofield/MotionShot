...
static void printUsage()
{
    cout << "Rotation model images stitcher.\n\n"
            ...
            "\nCompositing Flags:\n"
            "  --warp "
            "(affine|plane|cylindrical|spherical|fisheye|stereographic|compressedPlaneA2B1|"
            "compressedPlaneA1.5B1|compressedPlanePortraitA2B1|compressedPlanePortraitA1.5B1|"
            "paniniA2B1|paniniA1.5B1|paniniPortraitA2B1|paniniPortraitA1.5B1|mercator|"
            "transverseMercator)\n"
            ...
            "      Output warped images separately as frames of a time lapse movie, with 'fixed_' "
            "prepended to input file names.\n"
            "  --rangewidth <int>\n"
    ...
    for (int i = 1; i < argc; ++i) {
        if (string(argv[i]) == "--help" || string(argv[i]) == "/?") {
            ...
            if (string(argv[i + 1]) == "no" || string(argv[i + 1]) == "voronoi" ||
                string(argv[i + 1]) == "gc_color" || string(argv[i + 1]) == "gc_colorgrad" ||
                string(argv[i + 1]) == "dp_color" || string(argv[i + 1]) == "dp_colorgrad")
