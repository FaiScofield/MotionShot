    ...
    for (int i = 0; i < Gray.rows; i++) {
        for (int j = 0; j < Gray.cols; j++) {
            ...
            for (k = 0, matches = 0; matches < num_min_matches && k < num_samples; k++) {
                ...
                B_sam = samples_Frame[i][j][k][0];
                G_sam = samples_Frame[i][j][k][1];
                R_sam = samples_Frame[i][j][k][2];
                ...
                RGB_Vec = R_sam * R + G_sam * G + B_sam * B;
                RGB_Vec = pow(RGB_Vec, 2);
                ...
                for (int i_tmp = i_min; i_tmp <= i_max; i_tmp++) {
                    for (int j_tmp = j_min; j_tmp <= j_max; j_tmp++) {
                        if (!(i_tmp == i && j_tmp == j)) {
                            ...
                            int tmpGrad = Gray.at<uchar>(i, j) - Gray.at<uchar>(i_tmp, j_tmp);
                            tmpGrad = abs(tmpGrad);
                            if (tmpGrad >= maxGrad)
                                maxGrad = tmpGrad;
                        }
