...
namespace aip {
    ...
    public:
            ...
            const std::map<std::string, std::string> & options)
        {
            ...
            Json::Value result = this->request(_body_seg, null, data, null);
        ...
        /**
         * @brief body_seg_cv
         * 重载, 把输入格式从 stirng 改为 OpenCV Mat
         */
        Json::Value body_seg_cv(
            cv::InputArray image,
            const std::map<std::string, std::string> & options)
        {
            std::map<std::string, std::string> data;
            ...
            char* buf_char = reinterpret_cast<char*>(buf.data());
            data["image"] = aip::base64_encode(buf_char, (int)buf.size());
