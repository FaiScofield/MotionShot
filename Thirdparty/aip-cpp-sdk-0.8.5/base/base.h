...
protected:
    std::string getAccessToken()
    {
        ...
        int status_code = this->client.get(ACCESS_TOKEN_URL, &params, nullptr, &response);
...
public:
    ...
    AipBase(const std::string& app_id, const std::string& ak, const std::string& sk)
        : _app_id(app_id), _is_bce(false), ak(ak), sk(sk), useHttp2(false)
    {
        if (_app_id == "") {
        }
    }
    void set_use_http2(bool use_http2) { this->client.setUseHttp2(use_http2); }
    void set_is_bce() { _is_bce = true; }
    ...
    void setDebug(bool debug) { this->client.setDebug(debug); }

    std::string getAk() { return ak; }
    ...
    Json::Value request(std::string url, std::map<std::string, std::string> const& params,
                        std::string const& data, std::map<std::string, std::string> const& headers,
                        bool isRetry = false)
    {
        std::string response;
        Json::Value obj;
    ...
    Json::Value request(std::string url, std::map<std::string, std::string> const& params,
                        ...
                        std::map<std::string, std::string> const& headers, bool isRetry = false)
    {
        std::string response;
        Json::Value obj;
    ...
    void prepare_request(std::string url, std::map<std::string, std::string>& params,
