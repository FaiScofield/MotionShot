...
void url_parse(const std::string& url, std::map<std::string, std::string>& params)
{
    ...
    if (pos != -1) {
        int key_start = pos + 1, key_len = 0, val_start = 0;
...
std::string hmac_sha256(const std::string& src, const std::string& sk)
{
    const EVP_MD* evp_md = EVP_sha256();
    unsigned char md[EVP_MAX_MD_SIZE];
    unsigned int md_len = 0;

    if (HMAC(evp_md, reinterpret_cast<const unsigned char*>(sk.data()), (int)sk.size(),
             reinterpret_cast<const unsigned char*>(src.data()), src.size(), md, &md_len) == NULL) {
        return "";
    }
          ...
          std::map<std::string, std::string>& headers, std::string& ak, std::string& sk)
{
    ...
    std::stringstream ss;
    ss << "bce-auth-v" << __BCE_VERSION__ << "/" << ak << "/" << timestamp << "/" << __BCE_EXPIRE__;
