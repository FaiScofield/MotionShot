...
void GCApplication::saveFGDMask()
{
    if (image->empty() || winName->empty() || mask.empty())
        return;
...
{
    cv::CommandLineParser parser(argc, argv, "{@input| messi5.jpg |} {@scale| 1.0 |}");
    help();
    ...
    Mat imageScaled;
    if (abs(scale - 1.) > 1e-9)
        resize(image, imageScaled, Size(), scale, scale);
    else
        imageScaled = image;

    gcapp.setImageAndWinName(imageScaled, winName);
    gcapp.showImage();
